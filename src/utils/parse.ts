 ...
 * @example
 * ```ts
 * parseSiteUrl('example.local', true); // https://example.local
 * parseSiteUrl('example.local', false); // http://example.local
 * ```
 *
 * @param domain Site domain
 * @param ssl Does site have SSL
 * @returns Site URL
 */
export function parseSiteUrl(domain: Site['domain'], ssl: Site['ssl']) {
  return ssl ? `https://${domain}` : `http://${domain}`;
}
