...
export default function Background() {
  ...
  function startSpawning() {
    ...
    spawnIntervalRef.current = window.setInterval(() => {
      if (Math.random() < FALLING_LINE_PROBABILITY) {
        // 10% chance every 100ms = ~1 line per second
        spawnFallingLine();
      }
    }, 100);
  }
  ...
  const spawnFallingLine = useCallback(() => {
    ...
    const newLine: FallingLine = {
      ...
      height:
        Math.floor(
          Math.random() * (MIN_MAX_HEIGHT[1] - MIN_MAX_HEIGHT[0] + 1)
        ) + MIN_MAX_HEIGHT[0],
      className:
        fallingLineColors[Math.floor(Math.random() * fallingLineColors.length)],
    };
