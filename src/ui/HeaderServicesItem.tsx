...
export default function HeaderServicesItem({ service, emit }: { service: ServiceStatus, emit: (name: ServiceType, action: string) => Promise<void> }) {
    ...
    <li className="relative p-2">
      <Dialog open={service.lastError !== ''} onClose={clearServiceError}>
        <div className="text-center w-full">
          <h2 className="text-lg font-bold rounded-full bg-red-500 text-white p-2 text-center w-10 h-10 mx-auto">!</h2>
        </div>
        <p className="text-sm"><strong>{service.name}:</strong> {service.lastError}</p>
      </Dialog>
      <ServicesStatusIcon
        name={service.name}
      ...
      <div className="flex gap-[1px]">
        {!service.is_installed ?
          <Button className={btnCss} onClick={() => emit(service.name, "install")}>Install</Button> :
          <>
            {service.is_installed && !service.is_running && <Button className={btnCss} onClick={() => emit(service.name, "start")}>Start</Button>}
            {service.is_running && <Button className={btnCss} onClick={() => emit(service.name, "stop")}>Stop</Button>}
            ...
            <Button className={btnCss} onClick={() => emit(service.name, "restart")}>Restart Service</Button>
          </>}
