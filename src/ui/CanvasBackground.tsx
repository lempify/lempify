...
export default function CanvasBackground() {
  ...
  const lastNoiseDimensionsRef = useRef({
    width: 0,
    height: 0,
    isDark: isDark,
  });
  ...
  const generateNoiseImageData = useCallback(
  ...
  const drawNoise = useCallback(
    (ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement) => {
      ...
      if (needsRegeneration) {
        noiseImageDataRef.current = generateNoiseImageData(
          canvas.width,
          canvas.height
        );
  ...
  const drawFallingLines = useCallback(() => {
    ...
    fallingLines.forEach(line => {
      ...
      gradient.addColorStop(
        1,
        `rgba(${valueByTheme('50,50,50', '255,255,255')}, 1)`
      );
  ...
  const drawSpotlight = useCallback(
    (x: number, y: number) => {
      ...
      gradient.addColorStop(
        0,
        valueByTheme('rgba(255, 255, 255, 0)', 'rgba(10, 10, 10, 0)')
      );
      gradient.addColorStop(
        0.7,
        valueByTheme('rgba(235, 235, 235, 0.3)', 'rgba(30, 30, 30, 0.3)')
      );
      gradient.addColorStop(
        1,
        valueByTheme('rgba(220, 220, 220, 0.6)', 'rgba(0, 0, 0, 0.6)')
      );
  ...
  return (
    <canvas
      ref={canvasRef}
      className='w-full fixed top-[65px] left-64 bottom-0'
    ></canvas>
  );
}
