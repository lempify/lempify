...
export default function useWindowEvents() {
  ...
  useEffect(() => {
    const unlistenFocus = async () =>
      await getCurrentWindow().listen<any>('tauri://focus', () => {
        setLastWindowEvent('focus');
      });
    const unlistenBlur = async () =>
      await getCurrentWindow().listen<any>('tauri://blur', () => {
        setLastWindowEvent('blur');
      });
    const unlistenMove = async () =>
      await getCurrentWindow().listen<any>('tauri://move', () => {
        setLastWindowEvent('move');
      });
    const unlistenResize = async () =>
      await getCurrentWindow().listen<any>('tauri://resize', () => {
